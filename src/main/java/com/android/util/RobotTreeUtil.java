...
public class RobotTreeUtil {
		...
		for(int j=0;j<tiDevices.getItemCount();j++){
			for(int i=0;i<devices.length;i++){
				if(tiDevices.getItem(j).getData("sn").equals(devices[i].getSerialNumber())){
					tiDevices.getItem(j).setImage(new Image(display, ClassLoader.getSystemResourceAsStream("icons/devices.png")));
	...
	public static TreeItem addTree(Display display,Tree parent,String name){
		...
		if(parent != null){
			...
			column.setText(name);
			column.setImage(new Image(display, ClassLoader.getSystemResourceAsStream("icons/project.png")));
		}
		return column;
	}
	...
	public static TreeItem addTreeItem(Display display,TreeItem parent,String name){
		...
		if(parent != null){
			...
			if(name.endsWith(".dat"))
				column.setImage(new Image(display, ClassLoader.getSystemResourceAsStream("icons/unknow.png")));
			else if(name.endsWith("Devices"))
				column.setImage(new Image(display, ClassLoader.getSystemResourceAsStream("icons/devices.png")));
			else if(parent.getText().equals("Devices")){
				column.setImage(new Image(display, ClassLoader.getSystemResourceAsStream("icons/devices.png")));
			}else
				column.setImage(new Image(display, ClassLoader.getSystemResourceAsStream("icons/folder.png")));
		}
		return column;
	}
