...
public class CpuInfo extends Dialog implements Runnable {
    ...
    private void createTable(Composite rtComp) {
        ...
        timeseries = new TimeSeries(
            ...
            CpuInfo.class$org$jfree$data$time$Millisecond != null ? CpuInfo.class$org$jfree$data$time$Millisecond
                : (CpuInfo.class$org$jfree$data$time$Millisecond = CpuInfo
    ...
    public float getTotalCPUInfo(String cpuInfo, String activity) {
        float cpu = 0;
        BufferedReader reader = new BufferedReader(new StringReader(cpuInfo));
        String line = "";
        try {
            if (activity == null || activity.equals("")) {
                while ((line = reader.readLine()) != null) {
                    if (line.contains("TOTAL:")) {
                        cpu = getCPUInfoByPID(line);
                        break;
                    }
                }
            }
            ...
            Logger.getLogger(CpuInfo.class).error(e);
            return 0;
        }
        return cpu;
    }
            ...
            try {
                synchronized (this) {
                    ...
                    if (!currentDevice.equals("") && !isOpened) {
                        String DUMPSYS_CPUINFO = "adb -s " + currentDevice
                                                 + " shell dumpsys cpuinfo";
                        if (pid.trim().equals("")) {
                            String cpuInfo = AdbUtil.send(DUMPSYS_CPUINFO, 5000);
                        ...
                        disp.asyncExec(new Runnable() {
                            ...
                            public void run() {
                                ...
                                try {
                                    ...
                                    Logger.getLogger(CpuInfo.class).error("CPUINFO运行异常，", e);
                                }
                            }
                        });
